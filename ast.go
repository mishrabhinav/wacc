...
// includeFileMap holds the included files
var includeFileMap map[string]bool
...
func addFileToInclude(file string) {
	if includeFileMap == nil {
		includeFileMap = make(map[string]bool)
	}

	includeFileMap[file] = true
}
...
func ParseAST(wacc *WACC) (ast *AST, err error) {
	...
	case ruleWACC:
		...
		for _, include := range ast.includes {
			_, included := includeFileMap[include.file]
			if included {
			...
			// sanitized AST
			astIncl, inclErr := ParseAST(waccIncl)
			if inclErr != nil {
				fmt.Println(err.Error())
				os.Exit(100)
			}
