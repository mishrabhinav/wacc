...
func (m *IncludeFilesMap) Include(file string) {
	m.Lock()
	defer m.Unlock()
...
func appendIncludedFiles(ast *AST, includeFilesMap *IncludeFilesMap) {
	for _, include := range ast.includes {
		...
		// Look for further syntax errors, which were missed by
		// the grammar
		if retErrs := astIncl.CheckFunctionCodePaths(); len(retErrs) > 0 {
		...
		if typeErrs := astIncl.TypeCheck(); len(typeErrs) > 0 {
...
func ParseAST(wacc *WACC) (ast *AST, err error) {
	...
	case ruleWACC:
		...
		appendIncludedFiles(ast, includeFilesMap)
	}
